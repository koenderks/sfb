...
\section{Chapter 7: Testing proportions and distributions}
...
Expected number = Historical \% $\times$ Observed number
    ...
    Total & 124.4 (100\%) & 156.2 & 156.2 &  & 59.587 \tstrut\bstrut\\
    \cline{1-4} \cline{6-6}
    \end{tabular}
\end{center}
}
...
\answer{
    Since every season has an expected value above 5 you can do a Chi-squared test.
}
...
\answer{
    The calculated Chi-squared value of 59.587 is higher than the critical chi-squared value of 7.8. $H_0$ is rejected. The 2019 distribution is significantly different from the historical distribution. There is a 5\% risk of a type-I error.
}
...
\answer{
    ...
    \\
    \answercode{{\color{dataset}\# Chi-squared test: x = observations p = model distribution} \\
...
\answer{
    z-score: $\frac{p_2 - p_1}{s_p} = \frac{0.144 - 0.113}{0.514} = 0.612$ \\
    \\
...
\\
Note that \texttt{R} actually returns a Chi-squared value. Because it actually does a Chi-squared test it can in fact be used to test more than two proportions at the same time.
}
