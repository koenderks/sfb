...
\question{
    Write down the \concept{null hypothesis} $H_0$ and the \concept{alternative hypothesis} $H_1$ for a one-sided test where the researchers want to show that the caffeine \concept{mean} RER is lower than the placebo \concept{mean} RER.
}
...
The \concept{critical t-value} for 16 \concept{degrees of freedom} $(n_1 + n_2 - 2)$ for a left-tailed one-sided test is -1.746. \\
...
\codeblock{t.test(x = caffeine, y = placebo, alternative = \textquotesingle less\textquotesingle, mu = 0, \\
